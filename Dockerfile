...
WORKDIR /home/toae/src/YaraHunter
COPY . .
RUN make clean \
    && make all \
    && cd /home/toae \
    && git clone https://github.com/Sam12121/yara-rules


FROM debian:bookworm
LABEL MAINTAINER="Toae"
...
WORKDIR /home/toae/usr
COPY --from=builder /home/toae/yara-rules .
COPY --from=builder /usr/local/yara.tar.gz /usr/local/yara.tar.gz
COPY --from=builder /home/toae/src/YaraHunter/YaraHunter .
COPY --from=builder /home/toae/src/YaraHunter/config.yaml .

RUN cd /usr/local/ \
    && tar -xzf yara.tar.gz
WORKDIR /home/toae/output

ENTRYPOINT ["/home/toae/usr/YaraHunter", "-config-path", "/home/toae/usr", "-rules-path", "/home/toae/usr"]
CMD ["-h"]
